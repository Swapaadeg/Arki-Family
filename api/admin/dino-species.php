...
function handlePost($pdo) {
    ...
    try {
        ...
        $stmt->execute([
            ...
            ':types' => json_encode(array_values(array_map('intval', $input['types']))),
            ':stats' => json_encode(array_values($input['stats'])),
        ]);
...
function handleDelete($pdo) {
    ...
    try {
        $stmt = $pdo->prepare('SELECT name FROM dino_species WHERE id = ?');
        $stmt->execute([$id]);
        $row = $stmt->fetch();
        if (!$row) {
            sendJsonError('Espèce introuvable', 404);
        ...
        $stmt->execute([$id]);
        sendJsonResponse(['message' => "Espèce \"{$row['name']}\" supprimée"]);
    } catch (PDOException $e) {
...
function validateInput($input) {
    ...
    foreach ($input['types'] as $t) {
        if (!in_array((int) $t, [1, 2, 3, 4, 5, 6])) {
            sendJsonError('Type invalide: ' . $t, 400);
        }
    }
    if (!isset($input['stats']) || !is_array($input['stats']) || empty($input['stats'])) {
        sendJsonError('Au moins une stat est requise', 400);
    }
    foreach ($input['stats'] as $s) {
        if (!in_array($s, ['health', 'stamina', 'oxygen', 'food', 'weight', 'damage', 'crafting'])) {
